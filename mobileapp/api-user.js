...
export const registerUser = async ({ name, email, password }) => {
    try {
        const response = await axios.post('http://192.168.18.68:5000/api/users/register', {
            name,
            email,
            password,
            role: 'customer'
...
export const loginUser = async ({ email, password }) => {
    try {
        const response = await axios.post('http://192.168.18.68:5000/api/users/login', {
            email,
            password
        });
...
export const logoutUser = async () => {
    try {
        const response = await axios.post('http://192.168.18.68:5000/api/users/logout');
...
export const updateUser = async ({ name, email, phone, bio, photo }) => {
    try {
        const response = await axios.put('http://192.168.18.68:5000/api/users/updateuser', {
            name,
            email,
            phone,
            bio,
